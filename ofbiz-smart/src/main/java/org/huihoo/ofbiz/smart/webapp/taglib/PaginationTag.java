...
public class PaginationTag extends TagSupport {
  ...
  public int doStartTag() throws JspException {
    try {
      ...
      if (CommUtils.isNotEmpty(cPage)) {
        if (CommUtils.isNumber(cPage)) this.currentPage = Integer.valueOf(cPage);
      }
      String pageBody = renderPaginationBody();
      if (LOG.isDebugEnabled()) LOG.debug(pageBody);
      pageContext.getOut().println(pageBody);
    } catch (IOException e) {
      LOG.error("生成分页发生异常", e);
    }
