...
public class FlexibleLocation {
  ...
  /**
   * Find the location type descriptor for the passed location String; generally is all text before
   * the first ":" character. If no type descriptor is found, defaults to "classpath".
   */
   ...
   * 
   * Supports standard locationTypes like http, https, ftp, jar, & file Supports a classpath
   * location type for when desired to be used like any other URL Supports OFBiz specific location
   * types like ofbizhome and component Supports additional locationTypes specified in the
   * locationresolvers.properties file
   * 
  ...
  public static URL resolveLocation(String location, ClassLoader loader)
    ...
    if (resolver != null) {
      ...
      throw new MalformedURLException("Could not find a LocationResolver for the location type: "
              + locationType);
    }
  }
  ...
  public static String stripLocationType(String location) {
    ...
    // now remove any extra forward slashes, ie as long as the first two are forward slashes remove
    // the first one
