...
public class StringEscapeUtil {
  ...
  static class CsvEscaper extends CharSequenceTranslator {
      ...
      private static final char[] CSV_SEARCH_CHARS = 
          new char[] {CSV_DELIMITER, CSV_QUOTE, C.CR, C.LF};
      ...
      public int translate(final CharSequence input, final int index, final Writer out) throws IOException {
          ...
          if (CommUtil.containsNone(input.toString(), CSV_SEARCH_CHARS)) {
              out.write(input.toString());
          } else {
              out.write(CSV_QUOTE);
              out.write(CommUtil.replace(input.toString(), CSV_QUOTE_STR, CSV_QUOTE_STR + CSV_QUOTE_STR));
              out.write(CSV_QUOTE);
          }
  ...
  static class CsvUnescaper extends CharSequenceTranslator {
      ...
      public int translate(final CharSequence input, final int index, final Writer out) throws IOException {
          ...
          if ( CommUtil.containsAny(quoteless, CSV_SEARCH_CHARS) ) {
              // deal with escaped quotes; ie) ""
              out.write(CommUtil.replace(quoteless, CSV_QUOTE_STR + CSV_QUOTE_STR, CSV_QUOTE_STR));
          } else {
              out.write(input.toString());
          }
          return Character.codePointCount(input, 0, input.length());
