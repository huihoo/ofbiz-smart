...
abstract class OnCommittedResponseWrapper extends HttpServletResponseWrapper {
   ...
   * {@link #contentWritten} is larger than or equal the response is considered committed.
   */
  ...
  /**
   * Invoke this method to disable invoking {@link OnCommittedResponseWrapper#onResponseCommitted()}
   * when the {@link javax.servlet.http.HttpServletResponse} is committed. This can be useful in the
   * event that Async Web Requests are made.
   */
  ...
  /**
   * Implement the logic for handling the {@link javax.servlet.http.HttpServletResponse} being
   * committed
   */
  protected abstract void onResponseCommitted();

  /**
   * Makes sure {@link OnCommittedResponseWrapper#onResponseCommitted()} is invoked before calling
   * the superclass <code>sendError()</code>
   */
  @Override
  ...
  /**
   * Makes sure {@link OnCommittedResponseWrapper#onResponseCommitted()} is invoked before calling
   * the superclass <code>sendRedirect()</code>
   */
  @Override
  ...
  /**
   * Makes sure {@link OnCommittedResponseWrapper#onResponseCommitted()} is invoked before calling
   * the calling <code>getOutputStream().close()</code> or <code>getOutputStream().flush()</code>
   */
  @Override
  ...
  /**
   * Makes sure {@link OnCommittedResponseWrapper#onResponseCommitted()} is invoked before calling
   * the <code>getWriter().close()</code> or <code>getWriter().flush()</code>
   */
  @Override
  ...
  /**
   * Makes sure {@link OnCommittedResponseWrapper#onResponseCommitted()} is invoked before calling
   * the superclass <code>flushBuffer()</code>
   */
  @Override
  ...
  /**
   * Adds the contentLengthToWrite to the total contentWritten size and checks to see if the
  ...
  /**
   * Ensures {@link OnCommittedResponseWrapper#onResponseCommitted()} is invoked before calling the
   * prior to methods that commit the response. We delegate all methods to the original
   * {@link java.io.PrintWriter} to ensure that the behavior is as close to the original
  ...
  /**
   * Ensures{@link OnCommittedResponseWrapper#onResponseCommitted()} is invoked before calling
   * methods that commit the response. We delegate all methods to the original
   * {@link javax.servlet.ServletOutputStream} to ensure that the behavior is as close to the
   * original {@link javax.servlet.ServletOutputStream} as possible. See SEC-2039
   *
   * @author Rob Winch
   */
